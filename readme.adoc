...
. Processing nodes representing descendent entities, from which variables can be reduced.  This
  case only applies when a reduction derived variable is needed on the parent node.

Each of the child nodes (representing an ancestor or descendent entity) is tasked with providing
...
for a single row (key is dot-notated variable spec).
